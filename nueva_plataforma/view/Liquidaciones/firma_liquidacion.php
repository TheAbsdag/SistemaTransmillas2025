...
<div class="container">
  <div class="card card-main p-4">
    <h3 class="text-center mb-4">📄 Liquidación del Trabajador</h3>

    <iframe id="iframeDesprendible" name="iframeDesprendible"></iframe>

    <form id="formDesprendible" method="POST" action="/nueva_plataforma/view/Pdfs/DesprendibleLiquidacion.php" target="iframeDesprendible" style="display:none;">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header border-0">
        <h5 class="modal-title fw-semibold">Firma de Liquidación</h5>
      ...
      <div class="modal-footer border-0">
        <button id="guardarFirma" type="button" class="btn btn-success w-100 py-2">
          💾 Guardar Firma
        </button>
      </div>
    </div>
...
<script>
// === Inicialización de canvas y firma ===
const canvas = document.getElementById('signature-pad');
const ctx = canvas.getContext('2d');
let drawing = false;

// ⚙️ Redimensiona correctamente cuando el modal se muestra
...
// Obtener posición correcta (mouse o táctil)
function getPos(e) {
  const rect = canvas.getBoundingClientRect();
  if (e.touches) {
    return {
...
document.getElementById('guardarFirma').addEventListener('click', async () => {
  ...
  const res = await fetch('/nueva_plataforma/controller/FirmarLiquidacionController.php', { 
    method: 'POST',
    body: formData 
  });
...
// Enviar el PDF automáticamente al iframe al cargar
