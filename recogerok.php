...
// agregar campo a base de datos 
	...
	$sql8 = "UPDATE firma_clientes SET nombre='$param82', numero_documento='$param83',correo_electronico='$param84', telefono='$param85',enviar_whatsapp='$param86' WHERE tipo_firma='Recogida' and id_guia='$id_param2'";
	...
	$sql12 = "SELECT idservicios,ser_estado,cli_telefono,ser_consecutivo,ser_peso, ser_volumen, ser_valorseguro, ser_valor FROM `serviciosdia` WHERE  idservicios = '$id_param2'";			
	$DB1->Execute($sql12);
	$rw12 = mysqli_fetch_array($DB1->Consulta_ID);
	
	// Guardar Link de servicioi
	$res = guardarLinkServicio($DB, $id_param2, $imprimir, $idguia,$rw12[4],$rw12[5],$rw12[6],$rw12[7]);
	$linkGuia = $res['url'];
	if ($rw12 === false) {
	}else {
			...
			if ($telefono==$param85) {
				//Remitente
				enviarAlertaWhat($numguia,$telefono,24,$idservicio,$linkGuia);
			}else {
				//Remitente
				enviarAlertaWhat($numguia,$telefono,24,$idservicio,$linkGuia);
				// Quien Entrega
				enviarAlertaWhat($numguia,$param85,24,$idservicio,$linkGuia);
			}
...
$param5 = str_replace('&0&','&&', $param5);
	...
	// consulta si ya hay una firma 
	$sqlF = "SELECT * FROM firma_clientes WHERE tipo_firma='Entrega' and id_guia='$id_param2'";
	$DB1->Execute($sqlF);
	$fir=mysqli_fetch_row($DB1->Consulta_ID);
	...
	if($porcobrar==0){
		...
		echo$sql2="UPDATE `cuentaspromotor` SET `cue_idoperentrega`='$id_usuario2', `cue_fecha`='$fechatiempo', cue_estado='10' $cond0 $tranf where cue_idservicio=$id_param2";
		$DB1->Execute($sql2);		

		echo$sql3="UPDATE `guias` SET `gui_userecomienda`='$id_nombre2',`gui_fechaentrega`='$fechatiempo' WHERE `gui_idservicio`='$id_param2'";
		$DB->Execute($sql3);
		if($param19>=1){
		...
		echo$sql1="UPDATE `servicios` SET `ser_estentrega`='$param1',ser_fechafinal='$fechatiempo',ser_fechaguia='$fechatiempo',`ser_estado`='10',ser_img_entre='$foto' WHERE `idservicios`=$id_param2";
		if($tipopago>1 and $cambios==""){
			...
			echo$sql51="DELETE FROM `pagoscuentas` WHERE  pag_idservicio=$id_param2";
			$DB1->Execute($sql51);	

			echo$sql5="INSERT INTO `pagoscuentas`(`pag_tipopago`,`pag_cuenta`, `pag_valor`, `pag_idoperario`, `pag_idservicio`,`pag_guia`, `pag_estado`, `pag_fecha`,`pag_img_transaccion`) VALUES ('$tipopago','$cuenta','$param12','$id_usuario','$id_param2','$param11','Al cobro','$fechatiempo','$img_transaccion')";
			$DB1->Execute($sql5);
		}
		...
		if($tipopago>1){
			...
			echo$sql5="INSERT INTO `pagoscuentas`(`pag_tipopago`,`pag_cuenta`, `pag_valor`, `pag_idoperario`, `pag_idservicio`,`pag_guia`, `pag_estado`, `pag_fecha`,`pag_img_transaccion`) 
			VALUES ('$tipopago','$cuenta','$param12','$id_usuario','$id_param2','$param11','Pendiente X cobrar','$fechatiempo','$img_transaccion')";
			$DB1->Execute($sql5);
		}

		$st=2; 
		echo$sql2="UPDATE servicios SET ser_pendientecobrar=$st WHERE idservicios='$id_param2' ";
		$DB1->Execute($sql2);
		echo$sql1="UPDATE `cuentaspromotor` SET `cue_pendientecobrar`='$st',cue_idoperpendiente='$id_usuario'  $tranf where cue_idservicio=$id_param2";
	...
	echo$sql7="UPDATE `seguimientoruta` SET `seg_estado`='completado',`seg_fechafinalizo`='$fechatiempo',`seg_guia`='$param11' WHERE `seg_idservicio`='$id_param2'  AND  seg_tipo='Entrega' and seg_estado!='Cambioruta' AND seg_fecha like '%$fechaactual%'";
	$DB->Executeid($sql7); 
	
	// Preparar la consulta SQL para insertar los datos en la tabla firma_clientes
	$sql8 = "UPDATE firma_clientes SET  nombre='$param82', numero_documento='$param83',correo_electronico='$param84', telefono='$param85',enviar_whatsapp='$param86' WHERE tipo_firma='Entrega' and id_guia='$id_param2'";
	$idfirma=$DB->Executeid($sql8);
	...
	$sql12 = "SELECT idservicios,ser_estado,ser_telefonocontacto,ser_consecutivo,cli_telefono,ser_peso, ser_volumen, ser_valorseguro, ser_valor FROM `serviciosdia` WHERE  idservicios = '$id_param2'";			
	$DB1->Execute($sql12);
	$rw12 = mysqli_fetch_array($DB1->Consulta_ID);
	
	// Guardar Link de servicioi
	$res = guardarLinkServicio($DB, $id_param2, $imprimir, $idguia,$rw12[5],$rw12[6],$rw12[7],$rw12[8]);
	$linkGuia = $res['url']; // imprime la URL
	...
	if ($rw12 === false) {
			...
			$telefonoremi=$rw12[4];
				...
				if ($telefonoremi==$param85){
					//Remitente
					enviarAlertaWhat($numguia,$telefonoremi,24,$idservicio,$linkGuia);
				}else {
					//Remitente
					enviarAlertaWhat($numguia,$telefonoremi,24,$idservicio,$linkGuia);
					// Quien Entrega
...
function enviarAlertaWhat($numguia,$telefono,$tipo,$idservi,$imagen1=""){
		...
		$data = array(
			...
			// "telefono" => "3125215864",  // Número de teléfono 3160490959
			"tipo_alerta" => "$tipo",
			"id_guia" => "$idservi",
			"imagen1" => "$imagen1"
